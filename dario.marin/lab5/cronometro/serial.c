...
void serial_put_long_int(long int value, int num_digits) {
  ...
  if (num_digits < 1 || num_digits > MAX_LONG_DIGITS) num_digits = i;

  for (int j = num_digits - 1; j > -1; j--) serial_put_char(48 + buffer[j]);
}

void serial_put_int(int value, int num_digits) {
  if (num_digits > MAX_INT_DIGITS) num_digits = MAX_INT_DIGITS;
...
char serial_recibido(void) { /* COMPLETAR */ }
